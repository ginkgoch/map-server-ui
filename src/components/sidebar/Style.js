...
export class Style extends React.Component {
    ...
    _styleContent(style, onCloseButtonClick = this.props.onCloseButtonClick) {
        ...
        <div className="sidebar-item">
            ...
            <div>
                <EditButtons onCloseButtonClick={ onCloseButtonClick }></EditButtons>
            </div>
        </div>
        );
    }
                    ...
                    style.classBreaks.map(cb => {
                        return (
                            <Menu.Item key={cb.style.id} style={{height: "30px", lineHeight: "30px"}}>
                                {
                                    this._styleContent(cb.style, this._removeClassBreak(cb.style.id, style))
                                }
                            </Menu.Item>
                        );
                    })
            ...
            <SubMenu title={this._styleContent(style)} {...this._passThroughProps()}>
                {
                    style.items.map(cb => {
                        return (
                            <Menu.Item key={cb.style.id} style={{height: "30px", lineHeight: "30px"}}>
                                {
                                    this._styleContent(cb.style, this._removeValueItem(cb.style.id, style))
                                }
                            </Menu.Item>
                        );
                    })
